...
void SphereRenderer::onAttach() {
    ...
    Nest::ProgramAsset programAsset = Nest::AssetLoader::loadProgram(
        "Shaders/vstSphere.glsl",
        "Shaders/fstSphere.glsl"
    );
    ...
    Foundation::Memory verticesMemory = Foundation::Memory::copying(vertices.data(), sizeof(SphereVertex) * vertices.size());
    Foundation::Memory indicesMemory = Foundation::Memory::copying(indices.data(), sizeof(uint32_t) * m_countIndices);
    ...
    m_vertexBuffer = createVertexBuffer(verticesMemory, vertices.size() * sizeof(SphereVertex), vertexLayout);
    m_indexBuffer = createIndexBuffer(indicesMemory, BufferElementType::UnsignedInt, m_countIndices);
}
