...
private:
    ...
    struct DirLight {
        ...
        /// Четвертый компонент (w) обычно равен 0, чтобы указать, что это направление, а не позиция.
        glm::vec4 direction;

        /// Фоновый (окружающий) цвет света.
        /// Определяет, как свет влияет на объекты, даже если они не находятся под прямым воздействием света. Обычно это слабый цвет, который добавляется ко всем объектам в сцене.
        glm::vec4 ambient;

        /// Диффузный цвет света.
        /// Определяет, как свет влияет на объекты, когда они находятся под прямым воздействием света. Этот цвет умножается на диффузный цвет материала объекта.
        glm::vec4 diffuse;

        /// Зеркальный (отраженный) цвет света.
        /// Определяет, как свет влияет на блики на объектах.
        /// Этот цвет умножается на зеркальный цвет материала объекта и зависит от угла между направлением света и нормалью поверхности.
        glm::vec4 specular;
    };
    ...
    struct PointLight {
        ...
        /// Четвертый компонент (w) обычно равен 1, чтобы указать, что это позиция, а не направление.
        glm::vec4 position;

        /// Постоянный коэффициент затухания света.
        /// Определяет, насколько интенсивность света уменьшается с расстоянием.
        /// Этот коэффициент не зависит от расстояния и обычно используется для базового уровня затухания.
        glm::vec4 constant;
        ...
        /// Определяет, насколько интенсивность света уменьшается квадратично с увеличением расстояния. Этот коэффициент умножается на квадрат расстояния до источника света.
        glm::vec4 quadratic;

        /// Фоновый (окружающий) цвет света.
        /// Аналогично DirLight, определяет слабый цвет, который добавляется ко всем объектам в сцене.
        glm::vec4 ambient;

        /// Диффузный цвет света.
        /// Определяет, как свет влияет на объекты, когда они находятся под прямым воздействием света.
        glm::vec4 diffuse;
    ...
    struct SpotLight {
        ...
        /// Фоновый (окружающий) цвет света.
        /// Аналогично DirLight и PointLight, определяет слабый цвет, который добавляется ко всем объектам в сцене.
        glm::vec4 ambient;
