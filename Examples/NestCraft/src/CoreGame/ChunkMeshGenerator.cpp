...
Mesh* ChunkMeshGenerator::generateMesh(ChunksStorage *chunksStorage,
                                       int chunkIndexX,
                                       int chunkIndexY,
                                       int chunkIndexZ,
        ...
        for (int voxelIndexY = 0; voxelIndexY < Chunk::SIZE_Y; voxelIndexY++) {
            for (int voxelIndexZ = 0; voxelIndexZ < Chunk::SIZE_Z; voxelIndexZ++) {
                ...
                if (isAir(x, y, z + 1, chunksStorage)) {
                    ...
                    if (ambientOcclusion) {
                        // top
                        a = (isAir(x, y + 1, z + 1, chunksStorage) ? 0.0f : 1.0f) * ambientOcclusionFactor;
                        // bottom
                        b = (isAir(x, y - 1, z + 1, chunksStorage) ? 0.0f : 1.0f) * ambientOcclusionFactor;
                        // left
                        c = (isAir(x + 1, y, z + 1, chunksStorage) ? 0.0f : 1.0f) * ambientOcclusionFactor;
                        // right
                        d = (isAir(x - 1, y, z + 1, chunksStorage) ? 0.0f : 1.0f) * ambientOcclusionFactor;
                        // top left
                ...
                if (isAir(x, y + 1, z, chunksStorage)) {
                    ...
                    if (ambientOcclusion) {
                        // left
                        a = (isAir(x + 1, y + 1, z, chunksStorage) ? 0.0f : 1.0f) * ambientOcclusionFactor;
                        // right
                        b = (isAir(x - 1, y + 1, z, chunksStorage) ? 0.0f : 1.0f) * ambientOcclusionFactor;
                        // back
                        c = (isAir(x, y + 1, z + 1, chunksStorage) ? 0.0f : 1.0f) * ambientOcclusionFactor;
                        // front
                        d = (isAir(x, y + 1, z - 1, chunksStorage) ? 0.0f : 1.0f) * ambientOcclusionFactor;
                        // left back
                        e = (isAir(x + 1, y + 1, z + 1, chunksStorage) ? 0.0f : 1.0f) *
                            ambientOcclusionFactor;
                        // left front
