...
//  ImGui_ImplVulkan_LoadFunctions().) 2024-02-14: *BREAKING CHANGE*: Moved RenderPass parameter
...
//  PipelineRenderingCreateInfo structure. 2024-01-19: Vulkan: Fixed vkAcquireNextImageKHR()
//  validation errors in VulkanSDK 1.3.275 by allocating one extra semaphore than in-flight frames.
//  (#7236) 2024-01-11: Vulkan: Fixed vkMapMemory() calls unnecessarily using full buffer size
//  (#3957). Fixed MinAllocationSize handing (#7189). 2024-01-03: Vulkan: Added MinAllocationSize
//  field in ImGui_ImplVulkan_InitInfo to workaround zealous "best practice" validation layer.
//  (#7189, #4238) 2024-01-03: Vulkan: Stopped creating command pools with
...
void ImGui_ImplVulkan_RenderDrawData(
    ...
    if (wrb->FrameRenderBuffers == nullptr) {
        ...
        memset(
            wrb->FrameRenderBuffers, 0, sizeof(ImGui_ImplVulkan_FrameRenderBuffers) * wrb->Count
        );
    }
    ...
    if (draw_data->TotalVtxCount > 0) {
        ...
        size_t vertex_size = AlignBufferSize(
            draw_data->TotalVtxCount * sizeof(ImDrawVert), bd->BufferMemoryAlignment
        );
        size_t index_size = AlignBufferSize(
...
bool ImGui_ImplVulkan_Init(ImGui_ImplVulkan_InitInfo *info) {
    IM_ASSERT(
        g_FunctionsLoaded && "Need to call ImGui_ImplVulkan_LoadFunctions() if "
