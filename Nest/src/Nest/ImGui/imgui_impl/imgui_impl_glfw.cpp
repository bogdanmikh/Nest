...
//  2022-03-23: Inputs: Fixed a regression in 1.87 which resulted in keyboard modifiers GlfwEvents being
...
#    if defined(__EMSCRIPTEN__) ||                                                                 \
        defined(__SWITCH__                                                                         \
        ) // no Vulkan support in GLFW for Emscripten or homebrew Nintendo Switch
...
// - Because glfwPollGlfwEvents() process all windows and some GlfwEvents may be called outside of it, you
...
// Workaround: X11 seems to send spurious Leave/Enter GlfwEvents which would make us lose our position,
...
// GLFW may dispatch window pos/size GlfwEvents after calling glfwSetWindowPos()/glfwSetWindowSize().
...
#    endif
            ...
            for (int i = 0; i < IM_ARRAYSIZE(bd->KeyOwnerWindows); i++)
                if (bd->KeyOwnerWindows[i] == vd->Window)
                    ...
                    ); // Later params are only used for main viewport, on which this function is
                       // never called.
