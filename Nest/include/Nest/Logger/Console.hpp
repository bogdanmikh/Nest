...
public:
    ...
    inline void setVisible(bool condition) {
        m_showConsole = condition;
    }
    ...
    void addErrorF(std::string_view fmt, Args &&...args) {
        fmt::basic_memory_buffer<char, 250> buf;
        fmt::vformat_to(fmt::appender(buf), fmt, fmt::make_format_args(args...));
        m_console.addError(std::string(buf.data(), buf.size()));
    }

    template<typename... Args>
    void addTextF(std::string_view fmt, Args &&...args) {
        fmt::basic_memory_buffer<char, 250> buf;
        fmt::vformat_to(fmt::appender(buf), fmt, fmt::make_format_args(args...));
        m_console.addText(std::string(buf.data(), buf.size()));
    }

    void addError(const std::string &errorMessage) {
        m_console.addError(errorMessage);
    }

    void addText(const std::string &textMessage) {
        m_console.addText(textMessage);
    }
...
private:
    ...
    struct AppConsole {
        ...
        void addError(const std::string &errorMessage) {
            AddLog(("[error] " + errorMessage).c_str());
        }

        void addText(const std::string &textMessage) {
            AddLog(textMessage.c_str());
        }
        ...
        void Draw(const char *title, bool *p_open) {
            ...
            if (ImGui::BeginChild(
                ...
                // widgets. If you only want raw text you can use
                // ImGui::TextUnformatted(log.begin(), log.end()); NB- if you have thousands of
                // entries this approach may be too inefficient and may require user-side clipping
                // to only process visible items. The clipper will automatically measure the height
                // of your first item and then "seek" to display only items in the visible area. To
                ...
                // - That you have cheap random access to your elements (you can access them given
                // their index,
                //   without processing all the ones before)
                // You cannot this code as-is if a filter is active because it breaks the 'cheap
                // random-access' property. We would need random-access on the post-filtered list.
                // A typical application wanting coarse clipping and filtering may want to
                // pre-compute an array of indices or offsets of items that passed the filtering
                // test, recomputing this array when user changes the filter, and appending newly
                // elements as they are inserted. This is left as a task to the user until we can
