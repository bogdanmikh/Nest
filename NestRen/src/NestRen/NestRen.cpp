...
void initialize() {
    NESTREN_LOG("MIREN INIT BEGIN");
    NESTREN_LOG("ALLOCATING MIREN CONTEXT, {} BYTES", sizeof(Context));
...
#endif
    renderSemaphorePost();
    NESTREN_LOG("MIREN INIT END");
}

void terminate() {
    NESTREN_LOG("MIREN SHUTDOWN BEGIN");
    DELETE(Foundation::getAllocator(), s_context);
    s_context = nullptr;
    NESTREN_LOG("MIREN SHUTDOWN END");
}

FrameBufferHandle createFrameBuffer(FrameBufferSpecification specification) {
    NEST_ASSERT(s_context != nullptr, "MIREN NOT INITIALIZED");
